...
#include "l_jmpbuf.h"
/*--------------------------------------------------------------------------*/
extern "C" int  gnucap_main(int,const char*[]);
...
extern JMP_BUF env;
/*--------------------------------------------------------------------------*/
int gnucap_main(int argc, const char *argv[])
{
  {
    ...
    return 0; //??YellowDuck, supports batch processing only
  for (;;) {itested();
...
static void process_cmd_line(int argc, const char *argv[])
{
  ...
  if ((argc > 2) && (strcasecmp(argv[1], "-i") == 0)) {untested();
    ...
    CMD::cmdproc(cmdbuf);
      ...
      //??YellowDuck automatic redirect the output to a file
      sprintf(cmdbuf, "> %s.out", argv[2]);
      CMD::cmdproc(cmdbuf);
