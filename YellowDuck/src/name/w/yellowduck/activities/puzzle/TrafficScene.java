...
public class TrafficScene extends name.w.yellowduck.YDActLayerBase {
	private final int kTagHCar2               =10;
	private final int kTagHCar3               =11;
	private final int kTagVCar2               =12;
	private final int kTagVCar3               =13;

	private final int kParkingLotXY       =6;
	
    private float xGaragePos, yGaragePos;
    private float garageCellWidth, garageCellHeight;
    CGPoint ptLastTouched;
    private PolygonSprite movingCar;
	
	public static CCScene scene() {
	    CCScene scene = CCScene.node();
	    CCLayer layer = new TrafficScene();
	...
	public void onEnter() {
	    ...
	    xGaragePos=260.0f/800 * szWin.width;
	    yGaragePos=141.0f/520*szWin.height;
	    
	    garageCellWidth=240.0f/800*szWin.width/kParkingLotXY;
	    garageCellHeight=240.0f/520*szWin.height/kParkingLotXY;
	    //draw grids
	    for (int j=0;j<=kParkingLotXY;++j) {
	        float x0=xGaragePos;
	        float y0=yGaragePos + j * garageCellHeight;
	        float x1=xGaragePos + garageCellWidth * kParkingLotXY;
	        float y1=y0;
	    ...
	    for (int i = 0; i <= kParkingLotXY; ++i) {
	        float x0=xGaragePos+i*garageCellWidth;
	        float y0=yGaragePos;
	        float x1=x0;
	        float y1=yGaragePos+garageCellHeight*kParkingLotXY;
	...
	protected void initGame(boolean firstTime, Object sender) {
	    ...
	    String description=getParkingDescriptionStr();
	    String cars[]=description.split(",");
	    for (int m=0; m < cars.length; ++m) {
	    	 String oneCar=cars[m];
	    
	        int carId=oneCar.charAt(0);
	        int x=oneCar.charAt(1);
	        int y=oneCar.charAt(2);
	        
	        int carSize=0, tag=0;
	        if (carId >= 'A' && carId <= 'N')
	            carSize=2;
	        else if (carId >= 'O' && carId <= 'R')
	            carSize=3;
	        else if (carId == 'X') {
	            carSize=2;
	        }
	        int xCarSize=0, yCarSize=0;
	        int xCarPos=0, yCarPos=y-'1';
	        ...
	        if (x >= '1' && x <= '6') {
	            ...
	            tag= (carSize<=2) ? kTagVCar2:kTagVCar3;
	        }
	        else if (x >= 'A' && x <= 'F') {
	            //horizontal
	            xCarSize=carSize;
	            yCarSize=1;
	            xCarPos=x-'A';
	            tag=(carSize <= 2)?kTagHCar2:kTagHCar3;
	        }
	        yCarPos = kParkingLotXY - yCarPos - yCarSize;
	        CGRect rcCar=CGRect.make(xCarPos*garageCellWidth+xGaragePos, yCarPos*garageCellHeight+yGaragePos, xCarSize*garageCellWidth, yCarSize*garageCellHeight);
	        rcCar.origin.x += 1;
	        rcCar.origin.y += 1;
	        rcCar.size.width -= 2;
	        rcCar.size.height-=2;
	        
	        PolygonSprite carSprite=new PolygonSprite(4);
	        carSprite.setVertix(0, rcCar.origin);
	        carSprite.setVertix(1, CGPoint.ccp(rcCar.origin.x+rcCar.size.width, rcCar.origin.y));
	        carSprite.setVertix(2, CGPoint.ccp(rcCar.origin.x+rcCar.size.width, rcCar.origin.y+rcCar.size.height));
	        carSprite.setVertix(3, CGPoint.ccp(rcCar.origin.x, rcCar.origin.y+rcCar.size.height));
	        carSprite.setSolid(true);
	        carSprite.setClr(getCarColor(carId));
	        carSprite.setTag(tag);
	        if (carId=='X') {
	            carSprite.setUserData("RedCar");
	        }
	        super.addChild(carSprite, 1);
	        floatingSprites.add(carSprite);
	    }
	}
	private String getParkingDescriptionStr() {
	    ...
	    return DataList[(mLevel-1)*5+mSublevel];
	}
	private ccColor4F getCarColor(int ident)
	{
	    int clr=0;
	    switch (ident) {
	        case 'X':
	        ...
	        case 'K':
	            ...
	            break;
	        case 'O':
	            clr=0xFFFF00FF; //duplicated
	            break;
	        case 'P':
	            clr=0xFF80FFFF;//duplicated
	            break;
	        case 'Q':
	            clr=0x0000FFFF;
	            break;
	    ...
	    return new ccColor4F(1.0f*((clr>>24)& 0xff)/0xff, 1.0f*((clr>>16)& 0xff)/0xff, 1.0f*((clr>>8)& 0xff)/0xff, 1.0f*(clr& 0xff)/0xff);
	}
	
	public boolean ccTouchesBegan(MotionEvent event) {
		CGPoint p1 = CCDirector.sharedDirector().convertToGL(CGPoint.ccp(event.getX(), event.getY()));
	    movingCar=null;
        CGRect rcGarage=CGRect.make(xGaragePos, yGaragePos, garageCellWidth*kParkingLotXY, garageCellHeight*kParkingLotXY);
        if (rcGarage.contains(p1.x, p1.y)) {
            for (CCNode node : floatingSprites) {
                if (node.getTag() >= kTagHCar2) {
	...
	public boolean ccTouchesMoved(MotionEvent event) {
        ...
        int xMovement=(int)((p1.x-ptLastTouched.x)/garageCellWidth);
        int yMovement=(int)((p1.y-ptLastTouched.y)/garageCellHeight);
        if (xMovement >= 1 || xMovement <= -1 || yMovement >= 1 || yMovement <= -1) {
            if (movingCar.getTag() == kTagHCar2 || movingCar.getTag()==kTagHCar3) {
                yMovement=0;
            }
            else if (movingCar.getTag() == kTagVCar2 || movingCar.getTag()==kTagVCar3) {
                xMovement=0;
            }
            CGRect rcCar=movingCar.enclosedArea();
            CGRect rcGarage=CGRect.make(xGaragePos, yGaragePos, garageCellWidth*kParkingLotXY, garageCellHeight*kParkingLotXY);
            boolean valid=false; CGPoint ptCollision=CGPoint.ccp(0, 0); //collision point check
            if (xMovement > 0 && rcCar.origin.x+rcCar.size.width + garageCellWidth < rcGarage.origin.x + rcGarage.size.width + garageCellWidth/2) {
                //check collision
                ptCollision=CGPoint.ccp(rcCar.origin.x+rcCar.size.width + garageCellWidth/2, rcCar.origin.y+rcCar.size.height/2);
                valid=true;
            }
            else if (xMovement < 0 && rcCar.origin.x > rcGarage.origin.x+garageCellWidth/2) {
                ptCollision=CGPoint.ccp(rcCar.origin.x - garageCellWidth/2, rcCar.origin.y+rcCar.size.height/2);
                valid=true;
            }
            if (yMovement > 0 && rcCar.origin.y+rcCar.size.height + garageCellHeight < rcGarage.origin.y + rcGarage.size.height + garageCellHeight/2) {
                ptCollision=CGPoint.ccp(rcCar.origin.x+rcCar.size.width/2, rcCar.origin.y+rcCar.size.height+garageCellHeight/2);
                valid=true;
            }
            else if (yMovement < 0 && rcCar.origin.y > rcGarage.origin.y+garageCellHeight/2) {
                ptCollision=CGPoint.ccp(rcCar.origin.x+rcCar.size.width/2, rcCar.origin.y-garageCellHeight/2);
                valid=true;
            }
            if (valid) {
                for (CCNode node : floatingSprites) {
                    if (node.getTag() >= kTagHCar2) {
                        PolygonSprite carSprite=(PolygonSprite)node;
                        if (carSprite.hit(ptCollision)) {
                            valid=false;
                            break;
                        }
                    }
            ...
            if (valid) {
                ...
                movingCar.moveWithOffsetX(xMovement*garageCellWidth, yMovement*garageCellHeight);
                if (movingCar.getUserData()!=null) {
                    //the red car
                    boolean exited=false;
                    float xRight=movingCar.getVertix(1).x;
                    float yMid=(movingCar.getVertix(1).y+movingCar.getVertix(2).y)/2;
                    if (xRight > rcGarage.origin.x + rcGarage.size.width - garageCellWidth/2) {
                        if (yMid > rcGarage.origin.y + garageCellHeight * 3 && yMid < rcGarage.origin.y + garageCellHeight * 4) {
                            exited=true;
                        }
                    }
                    if (exited) {
